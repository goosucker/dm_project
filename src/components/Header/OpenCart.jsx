...
import { incrementCount, decrementCount, removeAllProducts } from "../../store/cartSlice";
...
export const OpenCart = ({ closeCart }) => {
    ...
    const handleOfferBtnClick = async () => {
        if (cartInfo.length === 0) {
            ...
            try {
                const updateCart = await axios.post(BASED_URL + '/cart/update', {
                    "data": 
                ...
                const submitOffer = await axios.post(BASED_URL + '/cart/submit', {}, {
                    withCredentials: true
                });
    ...
    if (cartInfo.length === 0) {
        return (
            <ul className={cartStyle}>
                <button className={closeBtnStyle} onClick={closeCart}>Закрыть корзину</button>
